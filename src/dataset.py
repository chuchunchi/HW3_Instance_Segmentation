...
This module provides datasets for instance segmentation models.
It handles loading and preprocessing of TIFF images and their masks.
"""
...
class CellDataset(Dataset):
    ...
    def __init__(self, root_dir: Union[str, Path],
        ...
        Args:
            ...
            transforms: transform to be applied on the image and target
        """
    ...
    def __getitem__(self, idx: int) -> \
        ...
        Returns:
            Tuple containing:
                ...
                - target: Dictionary containing:
                    ...
                    - masks: Tensor of shape (N, H, W) containing masks
        ...
        for class_id, filename in self.CLASS_FILENAMES.items():
            ...
            binary_mask = (tifffile.imread(
                str(mask_path)) > 0).astype(np.uint8)
        ...
        if not boxes:
            raise RuntimeError(
                f"All instances in {sample_dir} are degenerate (size=0).")

        # Convert to tensors
        image = torch.from_numpy(image.astype(np.float32) / 255.0)
        masks = torch.as_tensor(
            np.stack(valid_masks, axis=0), dtype=torch.uint8)
...
class TestImageDataset(Dataset):
    ...
    This dataset loads images for inference without requiring masks or labels.
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, str]:
        ...
        if image.ndim == 2:  # Grayscale
            image = torch.from_numpy(
                image).float().unsqueeze(0).repeat(3, 1, 1)
...
def collate_fn(
    batch: List[Tuple[torch.Tensor, Union[Dict[str, torch.Tensor], str]]]
) -> Tuple[List[torch.Tensor], List[Union[Dict[str, torch.Tensor], str]]]:
