...
def debug_predictions(args):
    ...
    for i, (images, image_ids) in enumerate(dataloader):
        ...
        print(
            ...
            {np.bincount(predictions[0]['labels'].cpu().numpy())}")
        print(
            f"Score range: [{predictions[0]['scores'].min():.3f},\
              {predictions[0]['scores'].max():.3f}]")
...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str,
                        required=True, help='Path to test images')
    parser.add_argument('--ckpt', type=str, required=True,
                        help='Path to model checkpoint')
    parser.add_argument('--output', type=str,
                        default='debug', help='Output directory')
